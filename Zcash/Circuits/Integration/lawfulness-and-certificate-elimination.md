...
* **#14–16, domain selection:** exact per-circuit synthesis summaries compose into a compact Action planner trace. The proved V1 planner endpoint gives `usedRows = 1779` and Halo2's minimal-domain rule then gives `domainExponent = 11`; the supported-domain theorem is a numerical corollary, so its `native_decide` owner has been removed throughout the trust boundary.
