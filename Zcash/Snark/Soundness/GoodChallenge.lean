...
reprogramming; stating `accX` over deployed `x`-squeeze events is the gate-transport follow-on,
nontrivial since the decoded `C` is recovered from post-`x` rewinds. Like the other per-hypothesis
exclusions (`z ≠ 0` and the `ξ`-recovery, `1/p` each), this `d / p` budget is stated per use site —
the `_xgood` rungs compose it into their own accept-measure hypothesis, while the cross-budget
composition stays with the "uncomposed budgets" note in `Zcash.Snark.Soundness.Forking.Oracle`. The
pinned decoded endpoints take a single canonical difference polynomial, so the budget is drawn once.
-/
