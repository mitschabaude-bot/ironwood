...
<div class="g"><div class="g-head"><span class="term">algebraic certificate</span><span class="anchor">AGM.Prover.AlgebraicDForkCert · Forking.Algebraic.algebraicForkCertAttempt_valid</span></div><div class="def">A deployed IPA fork certificate whose every round point carries coefficients over the complete original <code>(g,U,W)</code> basis. PR #56 computes it by recursively rewinding the algebraic Fiat–Shamir adversary, proves that it records real accepting runs, and packages it for PR #28’s computed opening-or-relation kernel.</div></div>
...
<div class="g"><div class="g-head"><span class="term">expected extraction time</span><span class="anchor">Forking.ExpectedRuns.recursiveAlgebraicFork_sum_runs_le_of_forkSpread</span></div><div class="def">Under <em>fork spread</em> — every reachable extractor node offers at least <code>σ₀</code> good challenges — the recursive extractor's expected adversary-run count over the uniform tape is at most <code>(6·|F|/(σ₀−1))ᵏ</code>: <code>(6/δ)ᵏ</code> for good sets of density <code>δ</code>, polynomial in the instance length. Proven by rank counting over uniformly sampled challenge orders plus fresh-child-tape marginalization, in exact natural-number counting. The unconditional AFK composition — pricing the sparse-fork slice a grinding FS adversary can force, via lazily-sampled tables — is the remaining efficiency floor.</div></div>
<div class="g"><div class="g-head"><span class="term">DL reduction bound</span><span class="anchor">Forking.Algebraic.ComputedAlgebraicFSFamily.binding_prob_le_of_generatorRO_textbookDL</span></div><div class="def">The random-slot accounting instantiated with the computed algebraic Fiat–Shamir producer. Binding is the computable mismatch between the effective accepted value — including the <code>z⁻¹·(multiU + ξ·sU)</code> shift from the adversary's declared <code>U</code> coefficients, zero for honest proofs — and the adversary's carried aggregate opening; never nonexistence of an opening, which holds vacuously at prime order. In the uniform generator-RO setup model, its probability is bounded by the recursive query loss, the <code>z = 0</code> slice, and <code>|basis|</code> times the textbook-DL advantage; a clean extracted opening on a mismatch run is converted to a commitment-collision relation rather than discarded.</div></div>
</section>
...
<div class="g"><div class="g-head"><span class="term">capstones</span><span class="anchor">Soundness/Vesta.lean · Forking/Algebraic.lean</span></div><div class="def">The <code>legacy_orchard_verifier_vesta_*</code> ladder preserves noncomputable, propositional opening and constraint forms across conditional, deployed, adaptive, and rewind models. The computed algebraic Fiat–Shamir endpoint carries representations through the adaptive adversary, recursively constructs PR #28’s deployed instance, and bounds the real binding event by extraction loss plus plain DL.</div></div>
